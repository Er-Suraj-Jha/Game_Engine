...
namespace primal::tools {
	namespace {
		...
		void process_normals(mesh& m, f32 smoothing_angle)
		{
			...
			const bool is_hard_edge{ XMScalarNearEqual(smoothing_angle,180.f,epsilon) };  // if, 179.99999 <= smoothing_angle <= 180.00001, true
			const bool is_soft_edge{ XMScalarNearEqual(smoothing_angle,0.f,epsilon) };    // If, -0.00001 <= smoothing_angle <= 0.00001 , true
				...
				for (u32 j{ 0 }; j < num_refs; ++j)
				{
					...
					if (!is_hard_edge)
					{
						for (u32 k{ j + 1 }; k < num_refs; ++k)
						{
							...
							if (!is_soft_edge)
							{
								// NOTE: We're accounting for the length of n1 in this caculation because
								//       it can possibly change in this loop iteration.We assume unit length
								//       for n2.
								//       cos(angle) = dot(n1,n2) / (|n1|*|m2|)
								XMStoreFloat(&cos_theta, XMVector3Dot(n1, n1) * XMVector3ReciprocalLength(n1));
							}
