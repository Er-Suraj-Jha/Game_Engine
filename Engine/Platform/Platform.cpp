...
namespace primal::platform {
	...
	namespace {
		...
		LRESULT CALLBACK internal_window_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
		{
			...
			return long_ptr ? ((window_proc)long_ptr)(hwnd, msg, wparam, lparam) :
		...
		void resize_window(window_id id, u32 width, u32 height)
		{
			...
			else
			{
				...
				//       when the user changes th escreen resolution.
	...
	window create_window(const window_init_info* const init_info /* = nullptr*/)
	{
		...
		if (info.hwnd)
		{
			...
			//Set in the "extra" bytes the pointer to the window callback functiom
