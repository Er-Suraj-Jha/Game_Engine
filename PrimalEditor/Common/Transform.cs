...
namespace PrimalEditor.Components
{
    ...
    class Transform : Component
    {
        ...
        public override IMSComponent GetMultiSelectionComponent(MSEntity mSEntity) => new MSTranform(mSEntity);
        public Transform(GameEntity owner) : base(owner)
        { }
    }

    sealed class MSTranform : MSComponent<Transform>
    {
        private float? _posX;
        public float? PosX
        {
        ...
        protected override bool UpdateComponents(string propertyName)
        {
            switch (propertyName)
            {
                ...
                case nameof(RotY):
                case nameof(RotZ):
                    SelectedComponents.ForEach(c => c.Rotation = new Vector3(_rotX ?? c.Rotation.X, _posY ?? c.Rotation.Y, _posZ ?? c.Rotation.Z));
                    return true;
                case nameof(ScaleX):
                case nameof(ScaleY):
                case nameof(ScaleZ):
                    SelectedComponents.ForEach(c => c.Scale = new Vector3(_scaleX ?? c.Scale.X, _posY ?? c.Scale.Y, _posZ ?? c.Scale.Z));
                    return true;
            }
            return false;
        }
        ...
        public MSTranform(MSEntity mSEntity) : base(mSEntity)
        {
            Refresh();
        }
